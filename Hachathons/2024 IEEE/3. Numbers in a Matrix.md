...
Alice has $N$ integer numbers between $0$ and $2^M - 1$. She decided to build a matrix $A$ of size $N \times M$, where the $i$-th row is equal to the binary representation of the $i$-th number. Given the matrix $A$, Alice can perform an unlimited number of operations of the type:
