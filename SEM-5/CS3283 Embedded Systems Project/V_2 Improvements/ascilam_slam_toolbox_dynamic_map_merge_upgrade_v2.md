...
# Final files (based on your current repo & agents on ports 8888/8889)

> These are complete drop‑in files. Keep your existing micro‑ROS firmware publishing to `/robot1/*` and `/robot2/*`. The launch keeps your two agents.
...
<package format="3">
  <name>multirobot_nav</name>
  <version>0.1.0</version>
  <description>Multi-robot navigation with slam_toolbox and dynamic map merge</description>
  <maintainer email="sansikasuhan5@gmail.com">Platinum-Saber</maintainer>
  <license>MIT</license>
  ...
  <!-- External executables we launch -->
  <exec_depend>slam_toolbox</exec_depend>
  <exec_depend>robot_localization</exec_depend>

  <!-- Python runtime helpers for map_merger -->
  <exec_depend>python3-numpy</exec_depend>
  <exec_depend>python3-yaml</exec_depend>
...
setup(
    ...
    description='Multi-robot navigation with slam_toolbox and dynamic map merge',
    license='MIT',
    ...
    entry_points={
        'console_scripts': [
            'robot_coordinator = multirobot_nav.robot_coordinator:main',
            'robot_controller = multirobot_nav.robot_controller:main',
            'multi_robot_slam = multirobot_nav.multi_robot_slam:main',  # kept for reference, not launched
            'map_merger = multirobot_nav.map_merger:main',              # NEW
        ],
    },
)
```

## `config/slam_common.yaml` (new)
```yaml
slam_toolbox:
  ros__parameters:
    # These are common defaults; frames and scan_topic are overridden per-robot in launch
    resolution: 0.05
    minimum_time_interval: 0.0
    max_laser_range: 12.0
    use_scan_matching: true
    ...
    loop_search_maximum_distance: 10.0
    use_slam_toolbox_pose_graph: true
    mode: "mapping"      # change to "localization" when using a saved map
    publish_tf: true      # publishes <ns>/map -> <ns>/odom
    # base_frame / odom_frame / map_frame / scan_topic will be injected from launch
```

## `config/ekf_common.yaml` (new)
```yaml
ekf_filter_node:
  ros__parameters:
    two_d_mode: true
    ...
    # Fuse x, y, yaw from wheel odometry
    odom0_config: [ true,  true,  false,
                    false, false, true,
                    false, false, false,
                    false, false, false,
    ...
    odom0_differential: false
    odom0_relative: false
    # base_link_frame / odom_frame / world_frame / odom0 will be injected from launch
```

## `config/map_merger.yaml` (new)
```yaml
map_merger:
  ros__parameters:
    global_frame: "map"
    ...
    # Initial guesses for local <ns>/map -> global /map (x y yaw)
    alignment:
      robot1: "0.0 0.0 0.0"
      robot2: "2.0 0.0 0.0"
...
## `multirobot_nav/map_merger.py` (new)
```python
#!/usr/bin/env python3
import math
import numpy as np
...
class MapMerger(Node):
    def __init__(self):
        ...
        # alignment dictionary: ns -> "x y yaw"
        self.align_spec: Dict[str, str] = self.get_parameter('alignment').value
        self.align_T: Dict[str, np.ndarray] = {ns: self._spec_to_T(self.align_spec.get(ns, '0 0 0')) for ns in self.robot_ns}

        # QoS to get latched last map and future updates
        map_qos = QoSProfile(
            reliability=ReliabilityPolicy.RELIABLE,
            durability=DurabilityPolicy.TRANSIENT_LOCAL,
            history=HistoryPolicy.KEEP_LAST,
        ...
        # Allow live alignment updates: ros2 param set /map_merger alignment.robot2 "dx dy yaw"
        self.add_on_set_parameters_callback(self._on_param_set)
    ...
    def _on_map(self, ns: str, msg: OccupancyGrid):
        ...
        self.latest_maps[ns] = (info, grid)
        self._recompose()

    def _world_to_global_ij(self, x: np.ndarray, y: np.ndarray):
        gi = np.floor((y - self.origin_y) / self.res).astype(np.int64)
        gj = np.floor((x - self.origin_x) / self.res).astype(np.int64)
        return gi, gj

    def _fuse(self, ns: str, info: MapMetaData, grid: np.ndarray, T_align: np.ndarray, canvas: np.ndarray):
        h, w = grid.shape
        known = grid >= 0
        if not np.any(known):
            return
        ...
        # local metric coords (cell centers)
        x_local = info.origin.position.x + (xi + 0.5) * info.resolution
        y_local = info.origin.position.y + (yi + 0.5) * info.resolution
        ...
        cur = canvas.ravel()
        cur_vals = cur[flat]
        # unknown -> assign, else max
        unknown = (cur_vals < 0)
        cur_vals[unknown] = vals[unknown]
        cur_vals[~unknown] = np.maximum(cur_vals[~unknown], vals[~unknown])
        cur[flat] = cur_vals

    def _recompose(self):
        canvas = np.full((self.H, self.W), -1, dtype=np.int8)
        for ns, (info, grid) in self.latest_maps.items():
            self._fuse(ns, info, grid, self.align_T.get(ns, np.eye(3)), canvas)
        self.global_grid = canvas
...
## `launch/slam_merge.launch.py` (new)
```python
#!/usr/bin/env python3
from launch import LaunchDescription
...
def per_robot_group(ns: str, slam_yaml: ParameterFile, ekf_yaml: ParameterFile):
    slam_overrides = {
        'base_frame': f'{ns}/base_link',
        'odom_frame': f'{ns}/odom',
        'map_frame':  f'{ns}/map',
        'scan_topic': f'/{ns}/scan',  # absolute path
    }
    ekf_overrides = {
        'base_link_frame': f'{ns}/base_link',
        'odom_frame': f'{ns}/odom',
...
def _expand_robots(context, *args, **kwargs):
    robots = LaunchConfiguration('robots').perform(context)
    robots = [s.strip() for s in robots.split(',') if s.strip()]

    slam_yaml = ParameterFile('config/slam_common.yaml', allow_substs=True)
    ekf_yaml  = ParameterFile('config/ekf_common.yaml', allow_substs=True)

    actions = [per_robot_group(ns, slam_yaml, ekf_yaml) for ns in robots]

    # Global map merger (root ns)
    actions.append(Node(
        package='multirobot_nav', executable='map_merger', name='map_merger',
        parameters=['config/map_merger.yaml'], output='screen'))
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument('robots', default_value='robot1,robot2',
                              description='Comma-separated robot namespaces'),
        OpaqueFunction(function=_expand_robots),
    ])
```

## `launch/multi_robot_launch.py` (replace)
```python
#!/usr/bin/env python3
from launch import LaunchDescription
from launch.actions import IncludeLaunchDescription
...
def generate_launch_description():
    ...
    # micro-ROS agents (match your firmware):
    agent_r1 = Node(
        package='micro_ros_agent', executable='micro_ros_agent',
        arguments=['udp4', '--port', '8888'], name='micro_ros_agent_robot1', output='screen')
    agent_r2 = Node(
        package='micro_ros_agent', executable='micro_ros_agent',
        arguments=['udp4', '--port', '8889'], name='micro_ros_agent_robot2', output='screen')

    # Base -> LiDAR static TFs only (slam_toolbox & EKF will publish the dynamic links)
    base_to_lidar_r1 = Node(
        package='tf2_ros', executable='static_transform_publisher', name='robot1_base_to_lidar',
        arguments=['0', '0', '0.10',  '0', '0', '0',  'robot1/base_link', 'robot1/lidar_link'])
    base_to_lidar_r2 = Node(
        package='tf2_ros', executable='static_transform_publisher', name='robot2_base_to_lidar',
        arguments=['0', '0', '0.10',  '0', '0', '0',  'robot2/base_link', 'robot2/lidar_link'])

    # Include SLAM + EKF per robot + dynamic map merger
    slam_merge = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(pkg_share, 'launch', 'slam_merge.launch.py')),
        launch_arguments={'robots': 'robot1,robot2'}.items()
    )

    # Your controllers & coordinator (unchanged)
    ctrl1 = Node(
        package='multirobot_nav', executable='robot_controller', name='controller',
        namespace='robot1', parameters=['config/robot_params.yaml'], output='screen')
    ctrl2 = Node(
    ...
    return LaunchDescription([
        agent_r1, agent_r2,
        base_to_lidar_r1, base_to_lidar_r2,
        slam_merge,
        ctrl1, ctrl2,
        coordinator,
    ])
...
# Bring up agents + slam_toolbox + EKF + dynamic merger + controllers
ros2 launch multirobot_nav multirobot_launch.py

# Live tune alignment while watching RViz
ros2 param set /map_merger alignment.robot2 "1.5 0.3 0.08"
```

**TF ownership recap**
- `slam_toolbox` (per robot) publishes `<ns>/map -> <ns>/odom`.
- `robot_localization` EKF publishes `<ns>/odom -> <ns>/base_link`.
- Keep only the static `base_link -> lidar_link` publishers you already had.
