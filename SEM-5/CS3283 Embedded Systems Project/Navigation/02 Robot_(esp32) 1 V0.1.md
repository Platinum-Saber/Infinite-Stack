```cpp
// ESP32_Robot1_Complete.ino
// Complete code for Robot 1 ESP32 with LD19 LiDAR and M-Bot integration
...
const int agent_port = 8888;            // micro-ROS agent port for Robot 1
...
// Custom WiFi transport functions
...
void check_wifi_status() {
    ...
    if (now - last_wifi_check > 10000) { // Check every 10 seconds
        if (WiFi.status() != WL_CONNECTED && wifi_connected) {
            ...
            wifi_connected = true;
            Serial.println("Robot 1 WiFi reconnected!");
            setup_micro_ros();
        }
        last_wifi_check = now;
    }
...
void setup_micro_ros() {
    ...
    ret = rclc_node_init_default(&node, "robot1_node", "", &support);
    if (ret != RCL_RET_OK) return;
...
void init_messages() {
    // Scan message
    scan_msg.header.frame_id.data = "robot1/lidar_link";
    scan_msg.header.frame_id.size = strlen("robot1/lidar_link");
    ...
    // Odometry message
    odom_msg.header.frame_id.data = "robot1/odom";
    odom_msg.header.frame_id.size = strlen("robot1/odom");
    odom_msg.header.frame_id.capacity = odom_msg.header.frame_id.size + 1;
    
    odom_msg.child_frame_id.data = "robot1/base_link";
    odom_msg.child_frame_id.size = strlen("robot1/base_link");
